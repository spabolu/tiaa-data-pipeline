...
def transform(dataframes):
    ...
    # 	•	Split a date-time column into separate year, month, day, and time attributes.
    # 	•	Extract a domain-specific attribute from a string (e.g., extract domain name from an email address).
    # 	•	Divide full names into first and last names.
    # 	•	Parse numerical values embedded in strings into separate numeric columns.
    llm_prompt_data_transformation_SplittingColumns = """
    I am going to loop through an array of data frames. I will provide the dataframe name and the head of each one.
    Don’t drop any unnamed columns. Assume I already imported the dataframes and their variables to directly use. Do not preprocess the data as it is already preprocessed.

    Generate a Python script and do NOT include markdown formatting, ```python, in the python script. This script is meant to for `Splitting Columns` (bold text). 
    ''Keep the overall structural integrity of the original dataframe (for example maintain the column headings & values).'' (bold text).
    ...
    for entry in dataframes:
        name, df = entry['name'], entry['dataframe']
        llm_prompt_data_transformation_SplittingColumns += f"\nDataframe Name: {name}\nHead:\n{df.head(10).to_string(index=False)}\n"

    response = ai.ask_llm(llm_prompt_data_transformation_SplittingColumns)
